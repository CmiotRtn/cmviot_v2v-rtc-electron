...
import { IVideoDeviceManagerImpl } from '../impl/ICmviotRtcEngineImpl';
import CmviotBaseTI from '../ti/CmviotBase-ti';
import CmviotMediaBaseTI from '../ti/CmviotMediaBase-ti';
import ICmviotRtcEngineTI from '../ti/ICmviotRtcEngine-ti';
...
export class RtcEngineExInternal extends IRtcEngineExImpl {
  ...
  override initialize(context: RtcEngineContext): number {
    if (CmviotEnv.webEnvReady) {
      // @ts-ignore
      window.CmviotEnv = CmviotEnv;
      if (CmviotEnv.CmviotRendererManager === undefined) {
        const { RendererManager } = require('../../Renderer/RendererManager');
        CmviotEnv.CmviotRendererManager = new RendererManager();
      }
    }
    CmviotEnv.CmviotRendererManager?.enableRender();
  ...
  override release(sync: boolean = false) {
    CmviotEnv.CmviotElectronBridge.ReleaseRenderer();
    CmviotEnv.CmviotRendererManager?.clear();
    CmviotEnv.CmviotRendererManager = undefined;
  ...
  protected override getApiTypeFromJoinChannel(
    ...
    if (CmviotEnv.CmviotRendererManager) {
      CmviotEnv.CmviotRendererManager.defaultRenderConfig.channelId = channelId;
    }
    ...
    const screenCaptureSources = result.map(
      (value: ScreenCaptureSourceInfo) => {
        if (!value.thumbImage?.buffer || !value.thumbImage?.length) {
          ...
          value.thumbImage!.buffer = CmviotEnv.CmviotElectronBridge.GetBuffer(
        ...
        if (!value.iconImage?.buffer || !value.iconImage?.length) {
          ...
          value.iconImage.buffer = CmviotEnv.CmviotElectronBridge.GetBuffer(
  ...
  override setupLocalVideo(canvas: VideoCanvas): number {
    ...
    return (
      CmviotEnv.CmviotRendererManager?.setupLocalVideo({
  ...
  override setupRemoteVideo(canvas: VideoCanvas): number {
    ...
    return (
      CmviotEnv.CmviotRendererManager?.setupRemoteVideo({
        videoSourceType: sourceType,
        channelId:
          CmviotEnv.CmviotRendererManager?.defaultRenderConfig?.channelId,
        uid,
        view,
        rendererOptions: {
          contentMode: renderMode,
  ...
  override destroyRendererByView(view: any) {
    CmviotEnv.CmviotRendererManager?.destroyRendererByView(view);
  }
  ...
  override destroyRendererByConfig(
    ...
    CmviotEnv.CmviotRendererManager?.destroyRenderersByConfig(
      videoSourceType,
      channelId,
      uid
    );
