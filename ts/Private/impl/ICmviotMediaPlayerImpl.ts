...
export class IMediaPlayerImpl implements IMediaPlayer {
  ...
  openWithCmviotCDNSrc(src: string, startPos: number): number {
    const apiType = this.getApiTypeFromOpenWithCmviotCDNSrc(src, startPos);
    const jsonParams = {
      src: src,
      startPos: startPos,
      toJSON: () => {
  ...
  protected getApiTypeFromOpenWithCmviotCDNSrc(
    src: string,
    startPos: number
  ): string {
  ...
  getCmviotCDNLineCount(): number {
    const apiType = this.getApiTypeFromGetCmviotCDNLineCount();
    const jsonParams = {};
    const jsonResults = callIrisApi.call(this, apiType, jsonParams);
    return jsonResults.result;
  }
  ...
  switchCmviotCDNLineByIndex(index: number): number {
    const apiType = this.getApiTypeFromSwitchCmviotCDNLineByIndex(index);
    const jsonParams = {
      index: index,
      toJSON: () => {
        return {
  ...
  getCurrentCmviotCDNIndex(): number {
    const apiType = this.getApiTypeFromGetCurrentCmviotCDNIndex();
    const jsonParams = {};
    const jsonResults = callIrisApi.call(this, apiType, jsonParams);
    return jsonResults.result;
  }
  ...
  enableAutoSwitchCmviotCDN(enable: boolean): number {
    const apiType = this.getApiTypeFromEnableAutoSwitchCmviotCDN(enable);
    const jsonParams = {
      enable: enable,
      toJSON: () => {
        return {
  ...
  renewCmviotCDNSrcToken(token: string, ts: number): number {
    const apiType = this.getApiTypeFromRenewCmviotCDNSrcToken(token, ts);
    const jsonParams = {
      token: token,
      ts: ts,
      toJSON: () => {
  ...
  protected getApiTypeFromRenewCmviotCDNSrcToken(
    token: string,
    ts: number
  ): string {
  ...
  switchCmviotCDNSrc(src: string, syncPts: boolean = false): number {
    const apiType = this.getApiTypeFromSwitchCmviotCDNSrc(src, syncPts);
    const jsonParams = {
      src: src,
      syncPts: syncPts,
      toJSON: () => {
  ...
  protected getApiTypeFromSwitchCmviotCDNSrc(
    src: string,
